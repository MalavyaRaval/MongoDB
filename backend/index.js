...
app.get("/search-notes/", authenticationToken, async (req, res) => {
  ...
  if (!query) {
    return res
      .status(400)
      .json({ error: true, message: "Search querry error" });
  }
  ...
  try {
    ...
    return res.json({
      error: false,
      notes: matchingNotes,
      message: "Notes matching query retive success",
    });
  } catch (error) {
    return res.status(500).json({
      error: true,
      message: "Internal querry error",
    });
  }
});
